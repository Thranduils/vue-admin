	...
	<section>
		...
		<el-col :span="24" class="toolbar" style="background: none; padding-bottom: 0px;">
			<el-form :inline="true" :model="filters">
				<el-form-item>
					<el-input v-model="filters.name" placeholder="请输入绿地公园名称"></el-input>
				</el-form-item>
				<el-form-item>
					<el-button type="primary" v-on:click="getQueryPark">查询</el-button>
				</el-form-item>
		...
		<el-table :data="users" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="title" label="绿地公园名称" min-width="120" >
			</el-table-column>
		...
		<!--编辑界面-->
		<el-dialog title="编辑绿地公园" v-model="editFormVisible" :close-on-click-modal="false" width="90%">
			<el-form :model="editForm" label-width="120px" :rules="editFormRules" ref="editForm">
				<el-row>
					<el-col :span="12">
						<el-form-item label="绿地公园名称：" prop="title" >
				...
				<el-row>
					...
					<el-col :span="12">
						<el-form-item label="游玩时间：">
							<el-input type="textarea" v-model="editForm.playTime"></el-input>
						</el-form-item>
					</el-col>
				</el-row>
				<el-row>
			...
			<!--新增界面-->
		<el-dialog title="新增绿地公园" v-model="addFormVisible" :close-on-click-modal="false">
			<el-form :model="addForm" label-width="120px" :rules="addFormRules" ref="addForm">
				...
				<el-row>
					...
					<el-col :span="12">
						<el-form-item label="开放时间：">
							<el-input type="textarea" v-model="addForm.playTime"></el-input>
						</el-form-item>
					</el-col>
				</el-row>
				<el-row>
...
<script>
	...
	import { getParkListPage, removePark, batchRemovePark, editPark, addPark, getParkEdit ,editParkAble} from '../../api/api';
	...
	export default {
		...
		methods: {
            ...
            editEnabled: function (id, isShow) {
				...
				/*设置签名参数  END   以及公共参数 */
				editParkAble(para,publicParams).then((res) => {
					console.log(res);
					if(res.code == 1000) {
						//NProgress.done();
						this.$message({
			...
			getPark() {
				...
				getParkListPage(para,publicParams).then((res) => {
					if(res.code == 1000){
						if(res !== null){
							//console.log(res)
			...
			handleDel: function (index, row) {
				...
				var publicParams =  util.publicParam(obj,util);
				/*设置签名参数  END   以及公共参数 */
					removePark(para,publicParams).then((res) => {
						this.listLoading = false;
						if(res.code == 1000) {
							this.$message({
								message: '删除成功',
			...
			handleEdit: function (id) {
				...
				var publicParams =  util.publicParam(obj,util);
				/*设置签名参数  END   以及公共参数 */
				getParkEdit(para,publicParams).then((res) => {
					if(res.code == 1000){
						if(res.result !== null){
							//console.log(res);
			...
			editSubmit: function () {
				...
				/*设置签名参数  END   以及公共参数 */
							...
							editPark(para,publicParams).then((res) => {
			...
			addSubmit: function () {
				...
				/*设置签名参数  END   以及公共参数 */
							//para.birth = (!para.birth || para.birth == '') ? '' : util.formatDate.format(new Date(para.birth), 'yyyy-MM-dd');
							addPark(para,publicParams).then((res) => {
			...
			batchRemove: function () {
				...
				var publicParams =  util.publicParam(obj,util);
				/*设置签名参数  END   以及公共参数 */
					batchRemovePark(para,publicParams).then((res) => {
						this.listLoading = false;
						if(res.code == 1000) {
							this.$message({
								message: '删除成功',
