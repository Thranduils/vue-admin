	...
	<section>
		...
		<el-col :span="24" class="toolbar" style="background: none; padding-bottom: 0px;">
			<el-form :inline="true" :model="filters">
				<el-form-item>
					<el-input v-model="filters.name" placeholder="请输入健康疗养名称"></el-input>
				</el-form-item>
				<el-form-item>
					<el-button type="primary" v-on:click="getQueryHealth">查询</el-button>
				</el-form-item>
		...
		<el-table :data="users" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="title" label="健康疗养名称" min-width="140" >
			</el-table-column>
		...
		<!--编辑界面-->
		<el-dialog title="编辑健康疗养" v-model="editFormVisible" :close-on-click-modal="false" width="90%">
			<el-form :model="editForm" label-width="120px" :rules="editFormRules" ref="editForm">
				<el-row>
					<el-col :span="12">
						<el-form-item label="健康疗养名称：" prop="title" >
			...
			<!--新增界面-->
		<el-dialog title="新增健康疗养" v-model="addFormVisible" :close-on-click-modal="false">
...
<script>
	...
	import { getHealthListPage, removeHealth, batchRemoveHealth, editHealth, addHealth, getHealthEdit } from '../../api/api';
	...
	export default {
		data() {
			return {
				...
				editFormRules: {
					title: [
						{ required: true, message: '请输入健康疗养名称', trigger: 'blur' }
					]
				},
				//编辑界面数据
				editForm: {
		...
		methods: {
            ...
            editEnabled: function (id, isShow) {
				...
				/*设置签名参数  END   以及公共参数 */
				editHealth(para,publicParams).then((res) => {
					console.log(res);
					if(res.code == 1000) {
						//NProgress.done();
						this.$message({
			...
			getHealth() {
				...
				// //NProgress.start();
				getHealthListPage(para,publicParams).then((res) => {
					if(res.code == 1000){
						if(res !== null){
							//console.log(res)
			...
			handleDel: function (index, row) {
				...
				var publicParams =  util.publicParam(obj,util);
				/*设置签名参数  END   以及公共参数 */
					removeHealth(para,publicParams).then((res) => {
						this.listLoading = false;
						if(res.code == 1000) {
							this.$message({
								message: '删除成功',
			...
			handleEdit: function (id) {
				...
				var publicParams =  util.publicParam(obj,util);
				/*设置签名参数  END   以及公共参数 */
				getHealthEdit(para,publicParams).then((res) => {
					if(res.code == 1000){
						if(res.result !== null){
							//console.log(res);
			...
			addSubmit: function () {
				...
				/*设置签名参数  END   以及公共参数 */
							//para.birth = (!para.birth || para.birth == '') ? '' : util.formatDate.format(new Date(para.birth), 'yyyy-MM-dd');
							addHealth(para,publicParams).then((res) => {
			...
			batchRemove: function () {
				...
				var publicParams =  util.publicParam(obj,util);
				/*设置签名参数  END   以及公共参数 */
					batchRemoveHealth(para,publicParams).then((res) => {
						this.listLoading = false;
