	...
	<section>
		...
		<el-col :span="24" class="toolbar" style="background: none; padding-bottom: 0px;">
			<el-form :inline="true" :model="filters">
				<el-form-item>
					<el-input v-model="filters.name" placeholder="请输入购物点名称"></el-input>
				</el-form-item>
				<el-form-item>
					<el-button type="primary" v-on:click="getQueryShopPoint">查询</el-button>
				</el-form-item>
		...
		<el-table :data="users" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="title" label="购物点名称" min-width="120" >
			</el-table-column>
		...
		<!--编辑界面-->
		<el-dialog title="编辑购物点" v-model="editFormVisible" :close-on-click-modal="false" width="90%">
			<el-form :model="editForm" label-width="120px" :rules="editFormRules" ref="editForm">
				<el-row>
					<el-col :span="12">
						<el-form-item label="购物点名称：" prop="title" >
			...
			<!--新增界面-->
		<el-dialog title="新增购物点" v-model="addFormVisible" :close-on-click-modal="false">
...
<script>
	...
	import { getShopPointListPage, removeShopPoint, batchRemoveShopPoint, editShopPoint, addShopPoint, getShopPointEdit } from '../../api/api';
	...
	export default {
		...
		methods: {
            ...
            editEnabled: function (id, isShow) {
				...
				/*设置签名参数  END   以及公共参数 */
				editShopPoint(para,publicParams).then((res) => {
					console.log(res);
					if(res.code == 1000) {
						//NProgress.done();
						this.$message({
			...
			getShopPoint() {
				...
				getShopPointListPage(para,publicParams).then((res) => {
					if(res.code == 1000){
						if(res !== null){
							//console.log(res)
			...
			handleDel: function (index, row) {
				...
				var publicParams =  util.publicParam(obj,util);
				/*设置签名参数  END   以及公共参数 */
					removeShopPoint(para,publicParams).then((res) => {
						this.listLoading = false;
						if(res.code == 1000) {
							this.$message({
								message: '删除成功',
			...
			handleEdit: function (id) {
				...
				var publicParams =  util.publicParam(obj,util);
				/*设置签名参数  END   以及公共参数 */
				getShopPointEdit(para,publicParams).then((res) => {
					if(res.code == 1000){
						if(res.result !== null){
							//console.log(res);
				...
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							editShopPoint(parpara,publicParamsa).then((res) => {
				...
				this.$refs.addForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							/*设置签名参数  END   以及公共参数 */
							//para.birth = (!para.birth || para.birth == '') ? '' : util.formatDate.format(new Date(para.birth), 'yyyy-MM-dd');
							addShopPoint(para,publicParams).then((res) => {
			...
			batchRemove: function () {
				...
				var publicParams =  util.publicParam(obj,util);
				/*设置签名参数  END   以及公共参数 */
					batchRemoveShopPoint(para,publicParams).then((res) => {
						this.listLoading = false;
						if(res.code == 1000) {
							this.$message({
								message: '删除成功',
