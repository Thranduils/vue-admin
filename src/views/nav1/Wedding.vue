	...
	<section>
		...
		<el-col :span="24" class="toolbar" style="background: none; padding-bottom: 0px;">
			<el-form :inline="true" :model="filters">
				<el-form-item>
					<el-input v-model="filters.name" placeholder="请输入婚纱摄影名称"></el-input>
				</el-form-item>
				<el-form-item>
					<el-button type="primary" v-on:click="getQueryWedding">查询</el-button>
				</el-form-item>
		...
		<el-table :data="users" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="title" label="婚纱摄影名称" min-width="120" >
			</el-table-column>
		...
		<!--编辑界面-->
		<el-dialog title="编辑婚纱摄影" v-model="editFormVisible" :close-on-click-modal="false" width="90%">
			<el-form :model="editForm" label-width="120px" :rules="editFormRules" ref="editForm">
				<el-row>
					<el-col :span="12">
						<el-form-item label="婚纱摄影名称：" prop="title" >
			...
			<!--新增界面-->
		<el-dialog title="新增婚纱摄影" v-model="addFormVisible" :close-on-click-modal="false">
			<el-form :model="addForm" label-width="120px" :rules="addFormRules" ref="addForm">
				<el-row>
					<el-col :span="12">
						<el-form-item label="美丽婚纱摄影：" prop="title" >
...
<script>
	...
	import { getWeddingListPage, removeWedding, batchRemoveWedding, editWedding, addWedding, getWeddingEdit ,editWeddingAble} from '../../api/api';
	...
	export default {
		...
		methods: {
            ...
            editEnabled: function (id, isShow) {
				...
				/*设置签名参数  END   以及公共参数 */
				// console.log(para);
				editWeddingAble(para,publicParams).then((res) => {
					console.log(res);
					if(res.code == 1000) {
						//NProgress.done();
						this.$message({
			...
			getWedding() {
				...
				getWeddingListPage(para,publicParams).then((res) => {
					if(res.code == 1000){
						if(res !== null){
							//console.log(res)
			...
			handleDel: function (index, row) {
				...
				var publicParams =  util.publicParam(obj,util);
				/*设置签名参数  END   以及公共参数 */
					removeWedding(para,publicParams).then((res) => {
						this.listLoading = false;
						if(res.code == 1000) {
							this.$message({
								message: '删除成功',
			...
			handleEdit: function (id) {
				...
				var publicParams =  util.publicParam(obj,util);
				/*设置签名参数  END   以及公共参数 */
				getWeddingEdit(para,publicParams).then((res) => {
					if(res.code == 1000){
						if(res.result !== null){
							//console.log(res);
				...
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							var publicParams =  util.publicParam(obj,util);
							/*设置签名参数  END   以及公共参数 */
							editWedding(para,publicParamspara).then((res) => {
				...
				this.$refs.addForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							/*设置签名参数  END   以及公共参数 */
							//para.birth = (!para.birth || para.birth == '') ? '' : util.formatDate.format(new Date(para.birth), 'yyyy-MM-dd');
							addWedding(para,publicParams).then((res) => {
			...
			batchRemove: function () {
				...
				var publicParams =  util.publicParam(obj,util);
				/*设置签名参数  END   以及公共参数 */
					batchRemoveWedding(para,publicParams).then((res) => {
						this.listLoading = false;
						if(res.code == 1000) {
							this.$message({
								message: '删除成功',
